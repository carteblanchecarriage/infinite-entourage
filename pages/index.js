    ...
    <div className='min-h-screen bg-white text-black font-mono'>
      <SiteHeader>
        {session ? (
          <>
            <div
              className={`flex items-center gap-2 border-2 border-black px-2 md:px-3 py-1 md:py-2 ${infiniteMode ? 'bg-purple-100 border-purple-500' : ''}`}
            >
              <span className='text-sm md:text-base font-bold'>
                {infiniteMode ? '∞' : credits}
              </span>
              <span className='text-xs md:text-sm text-gray-600'>
                {infiniteMode ? 'INFINITE' : 'CREDITS'}
              </span>
            </div>
            <Link
              href='/credits'
              className='text-sm md:text-base font-bold border-2 border-black px-2 md:px-3 py-1 md:py-2 hover:bg-black hover:text-white transition'
            >
              BUY
            </Link>
            <Link
              href='/account'
              className='hidden sm:block text-sm font-bold border-2 border-black px-2 md:px-3 py-1 md:py-2 hover:bg-black hover:text-white transition'
            >
              ACCOUNT
            </Link>
          </>
      ...
      <main className='max-w-4xl mx-auto p-4 md:p-6'>
        ...
        {showSignupPrompt && (
          <div className='mb-8 border-4 border-black bg-black text-white p-6'>
            ...
            <p className='text-lg mb-6'>
              Sign up to keep going. Credits start at $5 for 20 generations — no
              subscription, no commitment.
            </p>
            <div className='flex flex-col sm:flex-row gap-4'>
              <Link
                href='/login'
                className='text-center text-xl font-black py-4 px-8 border-4 border-white bg-white text-black hover:bg-black hover:text-white hover:border-white transition'
              >
                SIGN UP →
              </Link>
              <button
                onClick={() => setShowSignupPrompt(false)}
                className='text-center text-xl font-bold py-4 px-8 border-4 border-white hover:bg-white hover:text-black transition'
              >
                MAYBE LATER
              </button>
            </div>
            ...
            <div className='flex-1'>
              ...
              {!session && freeRemaining > 0 && (
                <p className='mt-4 text-base font-bold text-green-700 border-2 border-green-700 inline-block px-3 py-1'>
                  {freeRemaining} FREE GENERATION
                  {freeRemaining !== 1 ? 'S' : ''} REMAINING — NO SIGN UP NEEDED
                </p>
              )}
            </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-8'>
          <div className='border-4 border-black p-5'>
            ...
            <p className='text-sm text-gray-700'>
              Every output is a cutout PNG with no background — ready to drop
              straight into Photoshop, Lumion, or any render. What you see is
              what you paste.
            </p>
          </div>
          <div className='border-4 border-black p-5'>
            ...
            <p className='text-sm text-gray-700'>
              Say "elderly man sitting" or "young woman with a coffee". We
              handle the rest — outfit, age, pose, and look. No two generations
              are the same, so your renders feel lived-in, not cloned.
            </p>
          </div>
          <div className='border-4 border-black p-5'>
            <div className='text-2xl font-black mb-2'>
            ...
            <p className='text-sm text-gray-700'>
              Other AI tools give you a photo. We give you entourage. Three
              styles — realistic, illustration, and silhouette — so it matches
              your presentation, not just your mood board.
            </p>
          </div>
        </div>
        ...
        <div className='mb-8 border-4 border-black p-4 md:p-6'>
          ...
          <ul className='space-y-1 text-base md:text-lg font-bold'>
            ...
            <li
              className='cursor-pointer hover:bg-black hover:text-white p-2'
              onClick={() =>
                setPrompt('elderly woman sitting on bench reading newspaper')
              }
            >
            ...
            <li
              className='cursor-pointer hover:bg-black hover:text-white p-2'
              onClick={() => setPrompt('cyclist riding a road bike')}
            >
          ...
          <div className='mt-4 pt-4 border-t-2 border-gray-200 space-y-1'>
            <p className='text-xs text-gray-500'>
              TIP: DESCRIBE THE SUBJECT AND ACTION. SKIP BACKGROUNDS LIKE "ON
              STREET" OR "AGAINST WALL" — THEY MAKE CUTOUTS MESSIER.
            </p>
            <p className='text-xs text-gray-500'>
              TIP: ADD POSE AND VIEW — "WALKING", "SIDE VIEW", "FRONT VIEW" —
              FOR CLEANER RESULTS.
            </p>
          </div>
        </div>
        ...
        {/* STYLE */}
        <div className='mb-8'>
          <label className='block text-lg md:text-xl font-bold mb-4'>
            STYLE
          </label>
          <div className='flex flex-col sm:flex-row gap-2 sm:gap-4'>
            {styles.map((s) => (
              ...
              >
                {s.label}
                {s.disabled && <span className='block text-xs'>SOON</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        {!infiniteMode && !session && freeRemaining < 1 ? (
          <div className='border-4 border-black bg-black text-white p-6 text-center'>
            ...
            <p className='text-base mb-5 text-gray-300'>
              Sign up to keep going. Credits start at $5 for 20 generations.
            </p>
            <Link
              href='/login'
              className='inline-block text-lg md:text-xl font-black py-3 px-8 border-4 border-white bg-white text-black hover:bg-black hover:text-white hover:border-white transition'
            >
              SIGN UP →
            </Link>
          </div>
        ...
        {error && (
          <div
            className={`mt-4 p-3 md:p-4 border-4 border-black font-bold text-base md:text-xl ${error.includes('INFINITE') ? 'bg-purple-500 text-white' : 'bg-red-500 text-white'}`}
          >
            {error}
          </div>
        )}
        ...
        {gallery.length > 0 && (
          <div className='mt-12 border-4 border-black p-4 md:p-6'>
            <h2 className='text-xl md:text-2xl font-black mb-4'>
              YOUR GENERATIONS ({gallery.length})
            </h2>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const existing = JSON.parse(
                          localStorage.getItem('ie_feedback') || '[]',
                        );
