...
const SUBJECT_PATTERNS = {
  ...
  animal: {
    ...
    framing: 'complete animal fully visible, entire creature in frame, natural pose'
  },
  ...
  plant: {
    ...
    treeKeywords: ['tree', 'pine', 'oak', 'maple', 'birch', 'willow', 'palm', 'apple tree', 'cherry tree', 'lemon tree', 'fruit tree', 'evergreen', 'deciduous'],
    treeFraming: 'full tree with complete trunk and full canopy visible, entire tree from base to top, standing tree'
  },
...
const STYLE_MODIFIERS = {
  ...
  illustration: {
    ...
    vehicle: 'flat vector illustration, solid color blocks, minimal detail, clean edges, isometric or side view, architectural entourage style, muted colors',
  ...
  silhouette: {
    all: 'solid black silhouette, clean edges, high contrast, flat graphic shape, no interior detail, pure black fill'
  }
};
...
function hasSpecifiedDiversity(prompt) {
  ...
  return { hasRace, hasGender, hasAge, hasYouthTerm };
}
...
function buildEntouragePrompt(userPrompt, style = 'realistic') {
  ...
  // Special handling for trees - use stronger framing to ensure full tree is shown
  ...
  // Build final prompt: framing + enhanced user prompt + style + safety + context
  let fullPrompt = `${framing}, ${enhancedPrompt}, ${styleModifier}`;
