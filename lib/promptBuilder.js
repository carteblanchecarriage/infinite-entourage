...
const SUBJECT_PATTERNS = {
  person: {
    keywords: [
      ...
      'walking', 'standing', 'running', 'jogging', 'strolling', 'wandering', 'riding', 'cycling',
      'cyclist', 'cyclists', 'rider', 'riders', 'jogger', 'joggers', 'skater', 'skaters',
      'hiker', 'hikers', 'surfer', 'surfers', 'swimmer', 'swimmers', 'climber', 'climbers',
      'dancer', 'dancers', 'athlete', 'athletes',
    ...
    framing: 'full body person from head to feet, complete figure fully in frame, head to toe visible, entire body shown',
    framingDynamic: 'full body person from head to feet, complete figure fully in frame, head to toe visible, entire body shown, frozen in place like a statue, perfectly sharp, zero motion blur, crisp still photograph'
  },
...
// Action words that indicate dynamic poses — kept separate from SUBJECT_PATTERNS to avoid iteration errors
const PERSON_ACTIONS = ['running', 'jogging', 'jumping', 'dancing', 'walking', 'sitting', 'crouching',
  'leaning', 'stretching', 'playing', 'climbing', 'swimming', 'throwing', 'kicking',
  'reaching', 'bending', 'lying', 'sprinting', 'skating', 'surfing', 'hiking', 'cycling', 'riding'];
...
const DIVERSITY = {
  ...
  age: [
    'young adult', 'adult', 'middle-aged', 'elderly', 'senior', 'young professional'
  ],
...
const STYLE_MODIFIERS = {
  realistic: {
    person: 'photograph of a real person, natural skin tones and texture, realistic hair and clothing, sharp focus, DSLR photography, ultra detailed, lifelike, real human being, not CGI, not 3D render, not illustration, not plastic, not clay',
    animal: 'photograph of a real animal, natural fur and feather detail, sharp focus, wildlife photography, ultra detailed, lifelike, not CGI, not 3D render',
    vehicle: 'photograph of a real vehicle, natural paint and material detail, sharp focus, automotive photography, ultra detailed, not CGI, not 3D render',
    plant: 'photograph of a real plant, natural leaf and bark texture, sharp focus, botanical photography, ultra detailed, not CGI, not illustration',
    object: 'photograph of a real object, natural material detail, sharp focus, product photography, ultra detailed, not CGI, not 3D render',
    default: 'photograph, photorealistic, natural textures, sharp focus, ultra detailed, not CGI, not 3D render, not illustration'
  },

  illustration: {
    person: 'architectural entourage illustration, contemporary graphic design style, muted desaturated color palette, earth tones, clean confident linework, natural human proportions, minimal facial detail, subtle tonal depth, editorial quality, professional architectural presentation style, not cartoon, not anime, not flat icon',
    animal: 'architectural entourage illustration, contemporary graphic style, muted desaturated colors, clean linework, natural proportions, subtle tonal depth, editorial quality, not cartoon, not clipart',
    vehicle: 'architectural entourage illustration, contemporary graphic style, muted desaturated color palette, clean linework, side or three-quarter view, minimal interior detail, editorial quality, not cartoon, not photorealistic',
    plant: 'architectural entourage botanical illustration, contemporary graphic style, muted greens and earth tones, clean linework, natural botanical form, subtle tonal depth, editorial quality',
    object: 'architectural entourage illustration, contemporary graphic style, muted desaturated color palette, clean linework, minimal detail, editorial quality, not cartoon',
    default: 'architectural entourage illustration, contemporary graphic style, muted desaturated color palette, clean linework, editorial quality, not cartoon, not photorealistic'
  },

  silhouette: {
    all: 'pure solid black silhouette, completely uniform flat black fill with zero variation, no grey tones anywhere, no gradients, no shading, no texture, no outlines, no interior lines or detail of any kind, single-color black shape only, crisp clean edges, architectural drawing silhouette, black paper cutout, maximum contrast on white background'
  }
};

// Negative prompts by style to steer Flux away from common failure modes
const NEGATIVE_PROMPTS = {
  realistic: 'motion blur, blurry, out of focus, soft focus, bokeh on subject, cartoon, anime, illustration, painting, drawing, CGI, 3D render, plastic skin, clay, artificial, background, outdoor scene, indoor scene, landscape, architecture, furniture, ground, floor, text, watermark, logo, duplicate subjects, extra limbs, floating objects',
  illustration: 'motion blur, blurry, out of focus, photorealistic, photograph, cartoon, anime, manga, children\'s book style, clipart, flat icon, app icon, emoji, background scene, landscape, ground, text, watermark, extra limbs, duplicate subjects',
  silhouette: 'motion blur, blurry, out of focus, color, grey, gray, gradient, texture, shading, background, scene, ground, interior detail, outline only, hollow, text, watermark, duplicate subjects, extra limbs',
};
...
const CONTEXT_PROMPT = 'solid neutral grey background, soft diffused lighting, centered in frame, tack sharp focus, frozen still moment, absolutely no motion blur, perfectly crisp and detailed';
...
function buildEntouragePrompt(userPrompt, style = 'realistic') {
  ...
  const promptLower2 = userPrompt.toLowerCase();

  // Override framing for groups of people
  const groupTerms = ['group', 'crowd', 'people', 'couple', 'pair', 'friends', 'family', 'two', 'three', 'four', 'five'];
  const isGroup = subjectType === 'person' && groupTerms.some(t => promptLower2.includes(t));
  if (isGroup) {
    framing = 'group of people, multiple figures fully visible in frame, all subjects shown head to toe, complete figures, no cropping';
  }

  // Use dynamic framing when action words are present
  const isDynamic = subjectType === 'person' && !isGroup &&
    PERSON_ACTIONS.some(a => promptLower2.includes(a));
  if (isDynamic) {
    framing = SUBJECT_PATTERNS.person.framingDynamic;
  }

  // Detect camera perspective cues and reinforce them
  const perspectivePrompt = (() => {
    const p = promptLower2;
    if (p.includes('away from') || p.includes('from behind') || p.includes('back view') || p.includes('rear view') || p.includes('walking away') || p.includes('running away') || p.includes('jogging away'))
      return ', viewed from behind, back facing camera, rear view, posterior angle, not facing camera';
    if (p.includes('side view') || p.includes('profile') || p.includes('from the side'))
      return ', side profile view, lateral angle, 90 degrees to camera';
    if (p.includes('three quarter') || p.includes('3/4') || p.includes('three-quarter'))
      return ', three-quarter view angle';
    return '';
  })();
  ...
  const promptLower = userPrompt.toLowerCase();
  const surfaceMatch = promptLower.match(/(?:laying?|sitting|standing|on)\s+(?:on\s+)?(?:a\s+)?(\w+(?:\s+\w+)?)/);
  ...
  // Detect weapons in combat scenarios (sword fighting, etc.)
  const weaponKeywords = ['sword', 'swords', 'katana', 'nunchuck', 'nunchucks', 'staff', 'spear', 'axe', 'dagger', 'knife', 'bow', 'shield'];
  const hasWeapon = weaponKeywords.some(w => promptLower.includes(w));
  const hasCombat = /fighting|combat|duel|sparring|battling/.test(promptLower);

  let objectPrompt = '';

  // Handle riding separately — person is ON the vehicle, not holding it
  const ridingMatch = userPrompt.match(/(?:riding|on)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i);
  if (ridingMatch && subjectType === 'person') {
    const vehicle = ridingMatch[1];
    objectPrompt = `, riding a ${vehicle}, person and complete ${vehicle} both fully visible, frozen still pose, no motion blur, no extra objects, no floating parts`;
  } else if (hasWeapon || hasCombat) {
    const weaponMatch = userPrompt.match(/(?:sword|katana|nunchuck|staff|spear|axe|dagger|knife|bow)s?/i);
    if (weaponMatch) {
      objectPrompt = `, each holding single ${weaponMatch[0]} correctly, one weapon per person, no extra floating weapons, swords clearly in hands not floating nearby`;
    }
  } else {
    // Detect held/carried objects
    const holdingPatterns = [
      /(?:with|holding|carrying)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i,
      /(?:using|uses?)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i,
      /(?:pushing|push|pulling|pull)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i,
      /(?:fighting|swinging|wielding|brandishing)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i,
    ];
    for (const pattern of holdingPatterns) {
      const match = userPrompt.match(pattern);
      if (match) {
        objectPrompt = `, holding ${match[1]}, person plus ${match[1]} fully visible, no duplicate objects, no floating items`;
        break;
      }
    }
  }
  
  // Detect multi-person action scenes (fighting, running, etc.)
  const multiPersonTerms = ['two', 'three', 'couple', 'group', 'pair', 'friends', 'people', 'men', 'women', 'crowd'];
  const actionTerms = ['fighting', 'running', 'jumping', 'dancing', 'playing', 'wrestling', 'chasing', 'racing', 'competing'];
  const hasMultiplePeople = multiPersonTerms.some(term => promptLower.includes(term));
  const hasAction = actionTerms.some(term => promptLower.includes(term));
  const actionPrompt = (hasMultiplePeople && hasAction) 
    ? ', frozen action moment, sharp motion capture, crisp detail on both figures, dynamic pose clarity' 
    : '';
  let styleModifier;
  if (style === 'silhouette') {
  ...
  // Build final prompt: framing + enhanced user prompt + perspective + surface items + object items + action + style + safety + context
  let fullPrompt = `${framing}, ${enhancedPrompt}${perspectivePrompt}${surfacePrompt}${objectPrompt}${actionPrompt}, ${styleModifier}`;
