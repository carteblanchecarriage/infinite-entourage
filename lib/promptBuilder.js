...
const SUBJECT_PATTERNS = {
  person: {
    ...
    framing: 'full body person from head to feet, standing upright, complete figure fully in frame, head to toe visible, entire body shown, natural standing pose'
  },
...
function buildEntouragePrompt(userPrompt, style = 'realistic') {
  ...
  // Detect objects person is holding/using (watering can, umbrella, bicycle, etc.)
  const objectPatterns = [
    /(?:with|holding|carrying)\s+(?:a\s+)?(\w+(?:\s+\w+)?)/i,
    /(?:riding|rides?)\s+(?:a\s+)?(\w+)/i,
    /(?:using|uses?)\s+(?:a\s+)?(\w+)/i,
    /(?:pushing|push|pulling|pull)\s+(?:a\s+)?(\w+)/i,
    /(?:fighting|swinging|wielding|brandishing)\s+(?:a\s+)?(\w+)/i
  ];
  ...
  for (const pattern of objectPatterns) {
    ...
    if (match) {
      objectPrompt = `, holding single ${match[1]} in her hands at her side, person plus ${match[1]} fully visible, no duplicate objects, no floating items`;
      break;
    }
  }
  
  // Add weapon-specific guidance for combat scenarios
  if (hasWeapon || hasCombat) {
  ...
  // Build final prompt: framing + enhanced user prompt + surface items + object items + action + style + safety + context
  let fullPrompt = `${framing}, ${enhancedPrompt}${surfacePrompt}${objectPrompt}${actionPrompt}, ${styleModifier}`;
